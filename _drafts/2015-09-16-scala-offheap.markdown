...
В качестве отправной точки, мы будем использовать пример про 50 000 000  биржевых сделок из доклада Никиты Сальникова-Тарновского, как раз по этой тематике. Все примеры реализаций на *Java* вы сможете найти по [ссылке](https://github.com/iNikem/offheap), здесь же у нас будут варианты реализации на *Scala* и  результаты бенч марков для обеих версий. 

Перед тем как перейти к примеру еще немного терминов:
...
Все, а теперь непосредственно сам пример. 

Представим ситуацию на бирже, нам нужно осуществлять сделки, а затем их обрабатывать и получать некий результат. Кол-во этих сделок велико - предположим 50 000 0000 сделок в день.
...
Но есть одно но, писать такой код не удобно, ошибиться просто, а в случае с *Unsafe*, еще и не безопасно. Так что же такого может предложить *Scala*, чего нет в *Java*. И ответ на него - макросы. Ими и воспользовался Денис Шабалин из EPFL, создав проект *scala-offheap*.
