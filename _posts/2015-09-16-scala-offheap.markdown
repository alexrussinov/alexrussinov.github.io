...
comments: true
layout: post
title:  "Scala off-heap - своя колея"
...
Сразу хочу отметить, что кросс языковое измерение производительности дело странное и не благодарное по многим причинам. На эту тему есть отличная статья Алексея Шипилева - ["Java vs. Scala: Divided We Fail"](http://shipilev.net/blog/2014/java-scala-divided-we-fail/).
Целью же этого блога, является не столько желание сравнить производительность одного и того же кода в реализациях **Scala / Java**, сколько показать, что прикольного можно сделать на **Scala** и как эффективно это будет работать.
...
Настало время испытаний и напомню, что количество совершаемых сделок равно 50 000 000.
...
Тестирование у нас будет по фен Шую и для этого, мы воспользуемся **JMH (Java Microbenchmark Harness)**. Для **sbt** существует плагин, позволяющий запускать бенч марки непосредственно из под нее. Сейчас я приведу пример кода и в двух словах поясню, что происходит, а далее будут только результаты, т.к. различия лишь в части создаваемого экземпляра класса для тестов.
...
Раз уж с объектами работать так медленно, попробуем от них отказаться и остановим свой выбор на массиве примитивов.
...
Результаты *293.96 мс - Scala VS 300.945 мс - Java*. Как мы видим *Unsafe* быстрее *ByteBuffer* более чем в два раза и является бесспорным лидером в гонке за лидерство.
